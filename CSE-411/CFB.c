...
void AESEncryption(unsigned char * plainText, unsigned char * temp, unsigned char * expandedKey, unsigned char * cipher, int k)
{
    unsigned char * state = malloc(16);
    for (int i = 0; i < 16; ++i)
    {
     state[i] = temp[i] ^ expandedKey[i];
    }
    ...
    for(int i=0; i<16; i++){
        cipher[k*16 + i] ^= plainText[k*16 + i];     // Change for CBC
    }

    for(int i=0; i<16; i++){
        temp[i] = cipher[k*16 + i];
    }

    free(state);
}

// void AESDecryption(unsigned char * cipher, unsigned char * temp, unsigned char * expandedKey, unsigned char * plainText, int k)
// {
//     unsigned char * state = malloc(16);
//     //key whitening
//     for (int i = 0; i < 16; ++i)
...
//         plainText[k*16 + i] ^= temp[i];     // Change for CBC
//     }
...
void AESDecryption(unsigned char * plainText, unsigned char * temp, unsigned char * expandedKey, unsigned char * cipher, int k)
{
    unsigned char * state = malloc(16);
    for (int i = 0; i < 16; ++i)
    {
...
void generate_iv(unsigned char *iv) {
  // Seed the random number generator with current time (not ideal for production)
  srand(time(NULL));
...
// encrypting blocks
    for(int i=0; i<(filesize/16 + 1); i++){
        ...
        AESEncryption(plainText, temp, expandedKey, cipher, i);
    }
...
// deprypting blocks
    for(int i=0; i<(filesize/16 + 1); i++){
        ...
        AESDecryption(cipher, temp, expandedKey, decryptedText, i);
    }
