...
int main(){
    ...
    while(completed_processes < n){
        ...
        int high_priority = INT_MIN, high_priority_index = -1;
        for(int i=0; i<ready_processes.size(); i++){
            if(priority[ready_processes[i]] > high_priority){
                high_priority = priority[ready_processes[i]];
                high_priority_index = ready_processes[i];
            }
        }

        temp_burst_time[high_priority_index]--;
        current_time++;

        if(temp_burst_time[high_priority_index] == 0){
            completion_time[high_priority_index] = current_time;
            completed_processes++;
            ready_processes.erase(remove(ready_processes.begin(), ready_processes.end(), high_priority_index), ready_processes.end());
        }
    }
